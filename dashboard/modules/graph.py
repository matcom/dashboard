...
from modules.utils import (
    count_publications_between_two_persons,
    count_publications_by_person,
    count_theses_between_two_advisors,
    count_theses_by_advisor,
    darken_color,
    select_color,
)
...
class EdgeGraph:
    def __init__(
        self, source: NodeGraph, target: NodeGraph, info, color="#ACDBC9"
    ) -> None:
        self.source = source
        self.target = target
        self.info = info
        ...
        for advisor in thesis.advisors:
            for advisor_2 in thesis.advisors:
                if advisor == advisor_2:
                    continue
                count_thesis = count_theses_between_two_advisors(
                    theses, advisor, advisor_2
                )
                edges.append(
                    Edge(
                        source=advisor,
...
def build_nodes_and_edges(
    publications: any, color: Tuple[str, str]
) -> Tuple[List[NodeGraph], List[EdgeGraph]]:
    ...
    for publication in publications:
        ...
        for i in range(len(publication.authors)):
            ...
            for j in range(i + 1, len(publication.authors)):
                ...
                edges.append(
                    EdgeGraph(
                        all_nodes[author.uuid], all_nodes[author_2.uuid], publication
                    )
                )
...
def build_publications_graph(
    publications: List[any], color: Tuple[str, str], width=900, height=700
) -> any:
    ...
    for edge in edgesGraph:
        count_pub = count_publications_between_two_persons(
            publications, edge.source.info, edge.target.info
        )
        edges.append(
            Edge(
